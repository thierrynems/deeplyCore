...
logging.basicConfig(format='%(levelname)s: %(name)s - %(message)s',
                    level=logging.INFO)
logger = logging.getLogger('DNN-Deeply') 
...
class BuildDNNModel(object):
    ...
    def __init__(self, data, bins, f_tp, f_fp, f_th,iteration,expName,model_dir):
        ...
        self.evaluationInfo = buildAndRunModel(self.dataDict, int(self.numberOfFeatEachBin), self.bins,
                                                self.numberOfClasses, f_tp, f_fp, f_th,iteration,expName,model_dir)
...
def buildAndRunModel(dataDict, numberOfFeatEachBin, bins, numberOfClasses, f_tp, f_fp, f_th,iteration,expName,model_dir):
    ...
    #filePath = 'weights.best.hdf5' # init path
    filePath = os.path.join(model_dir, expName +'_weights.best.hdf5') # our modification
   ...
   # with open("deeply-model.json", "w") as json_file:
   ...
   # sys.exit()
    ...
    #one time save
    fn = os.path.join(model_dir, 'deeply-model.json')
    ...
    model_file = os.path.join(model_dir, expName + str(iteration) + 'deeply-model.h5')
